...
export async function DELETE(request: Request) {
    try {
        ...
        return NextResponse.json({ error: { message: 'Erro ao excluir certificado e-CNPJ' } }, { status: 500 });
    }
}
...
export async function GET() {
    try {
        ...
        if (err instanceof ForbiddenError) return NextResponse.json(formatErrorResponse(err), { status: err.statusCode });
        return NextResponse.json({ error: { message: 'Erro ao buscar certificado e-CNPJ' } }, { status: 500 });
    }
}
